...
// Only fetch data if both course and unit are selected
if (!empty($courseCode) && !empty($unitCode)) {
    // Fetch student records with names
    $studentRows = fetchStudentRecordsWithNames($courseCode, $unitCode);
    
    // Fetch distinct dates for the selected course and unit
    $distinctDatesQuery = "SELECT DISTINCT dateMarked FROM tblattendance WHERE course = :courseCode AND unit = :unitCode ORDER BY dateMarked DESC";
...
// NEW FUNCTION: Fetch student records with names - FIXED TABLE NAMES
function fetchStudentRecordsWithNames($courseCode, $unitCode) {
    global $pdo;
    
    // First, let's check what tables actually exist in the database
    try {
        // Try to get students from the correct table structure
        // Based on common patterns, let's try these table names:
        $query = "SELECT 
                    s.registrationNumber as studentRegistrationNumber,
                    CONCAT(s.firstName, ' ', s.lastName) as studentName
                  FROM tblstudents s
                  WHERE s.course = :courseCode 
                  AND FIND_IN_SET(:unitCode, s.units) > 0
        ...
        // If that fails, try alternative table structures
        error_log("First query failed: " . $e->getMessage());
        
        // Alternative 1: Check if students are linked via a different table
        try {
            ...
            error_log("Second query failed: " . $e2->getMessage());
            
            // Last resort: Just get registration numbers from attendance records
            try {
                $query = "SELECT DISTINCT
                ...
                error_log("All queries failed: " . $e3->getMessage());
                return [];
            }
        }
    }
    ...
    <section class="main">
        ...
        <div class="main--content">
            ...
            <div class="page-header">
                ...
                <p>View attendance data for courses and units</p>
            </div>
            ...
            <div class="control-panel">
                <form method="GET" action="" id="attendanceForm">
                    ...
                    <div class="action-buttons">
                        <button type="submit" class="btn btn-view" id="viewRecordsBtn">
            ...
            <div class="selection-info" id="selectionInfo">
                <div class="info-grid">
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value"><?php echo count($studentRows); ?> Students</div>
                    </div>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="table-container">
                <div class="table-header">
                    ...
                    <div class="table-stats">
                        <span><?php echo count($studentRows); ?> Students</span>
                ...
                <div class="table attendance-table" id="attendaceTable">
                    <?php if (!empty($courseCode) && !empty($unitCode) && !empty($studentRows)): ?>
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            foreach ($studentRows as $row) {
                                ...
                                foreach ($distinctDatesResult as $dateRow) {
                                    ...
                                    // Display attendance status with appropriate badge
                                    if ($attendanceResult) {
                                        $status = $attendanceResult['attendanceStatus'];
                                        $badgeClass = 'status-' . strtolower($status);
                                        echo "<td><span class='attendance-badge $badgeClass'>" . $status . "</span></td>";
                                    } else {
                                        echo "<td><span class='attendance-badge status-absent'>Absent</span></td>";
                                    }
                                }
                    ...
                    <?php elseif (!empty($courseCode) && !empty($unitCode) && empty($studentRows)): ?>
                    ...
                    <div class="empty-state" id="emptyState">
                        ...
                        <p>Please select a course and unit, then click "View Records" to see attendance data</p>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateButtonStates() {
            const hasSelection = courseSelect.value && unitSelect.value;
            viewRecordsBtn.disabled = !hasSelection;
        }

        // Update button states when selections change
        courseSelect.addEventListener('change', updateButtonStates);
        unitSelect.addEventListener('change', updateButtonStates);
